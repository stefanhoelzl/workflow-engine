...
// on globalThis; forwards serialized payload to the __reportError host bridge
// only if the event was not default-prevented.
//
...
//
// Capture-and-delete pattern: __reportError is captured into the module's
// closure at eval time (which becomes the IIFE closure in the bundled
// output), then the global is deleted so guest code cannot read or
// overwrite the raw host bridge. Required by CLAUDE.md §2 rule:
// "NEVER add a `__*`-prefixed global to the sandbox without a
// capture-and-delete shim".

const _report = (globalThis as { __reportError?: (payload: unknown) => void })
	.__reportError;
...
delete (globalThis as { __reportError?: unknown }).__reportError;
