...
function assertPositiveInt(value: unknown, path: string): number {
	if (typeof value !== "number" || !Number.isInteger(value) || value <= 0) {
		throw new Error(`mail.send: ${path} must be a positive integer`);
	}
	return value;
}
...
function assertNonEmptyString(value: unknown, path: string): string {
	if (typeof value !== "string" || value.length === 0) {
		throw new Error(`mail.send: ${path} must be a non-empty string`);
	}
	return value;
}
...
function assertRecipient(value: unknown, path: string): Recipient {
	if (typeof value === "string") {
		if (value.length === 0) {
			throw new Error(`mail.send: ${path} must be non-empty`);
		}
		return value;
	}
	if (Array.isArray(value)) {
		if (value.length === 0) {
			throw new Error(`mail.send: ${path} must not be an empty array`);
		}
		for (const [i, entry] of value.entries()) {
			if (typeof entry !== "string" || entry.length === 0) {
				throw new Error(`mail.send: ${path}[${i}] must be a non-empty string`);
			}
		}
		return value as readonly string[];
	}
	throw new Error(`mail.send: ${path} must be a string or array of strings`);
}
...
function assertOptionalString(
	...
	if (typeof value !== "string") {
		throw new Error(`mail.send: ${path} must be a string if provided`);
	}
	return value;
}
...
function assertSmtpConfig(raw: unknown): SmtpConfigWire {
	if (raw === null || typeof raw !== "object") {
		throw new Error("mail.send: smtp must be an object");
	}
	...
	if (tls !== "tls" && tls !== "starttls" && tls !== "plaintext") {
		throw new Error(
			'mail.send: smtp.tls must be one of "tls" | "starttls" | "plaintext"',
		);
	}
	if (o.auth === null || typeof o.auth !== "object") {
		throw new Error("mail.send: smtp.auth must be an object");
	}
	...
	if (typeof auth.pass !== "string") {
		throw new Error("mail.send: smtp.auth.pass must be a string");
	}
	const config: {
		host: string;
		port: number;
...
function assertAttachments(
	...
	if (!Array.isArray(raw)) {
		throw new Error("mail.send: attachments must be an array");
	}
	const out: MailAttachmentWire[] = [];
	for (const [i, entry] of raw.entries()) {
		if (entry === null || typeof entry !== "object") {
			throw new Error(`mail.send: attachments[${i}] must be an object`);
		}
		...
		if (typeof a.content !== "string") {
			throw new Error(
				`mail.send: attachments[${i}].content must be a base64 string`,
			);
		}
...
function assertMailOpts(raw: unknown): MailOptsWire {
	if (raw === null || typeof raw !== "object") {
		throw new Error("mail.send: options must be an object");
	}
	const o = raw as Record<string, unknown>;
	const opts: {
		smtp: SmtpConfigWire;
...
function mapTlsMode(tls: "tls" | "starttls" | "plaintext"): {
	...
	switch (tls) {
		...
		default:
			throw new Error(`mail.send: unreachable tls mode ${String(tls)}`);
	}
}
...
// `command` narrows message-data rejection vs RCPT-TO rejection.
...
function classifyMailError(err: unknown): {
	kind: MailErrorKind;
	code?: string;
	responseCode?: number;
	response?: string;
...
function throwStructured(err: unknown): never {
	...
	Object.assign(thrown, classified);
	(thrown as { name: string }).name = "MailError";
	throw thrown;
}
