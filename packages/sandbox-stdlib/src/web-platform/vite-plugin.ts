...
// packages/sandbox-stdlib/src/web-platform/source/entry.ts into a single
// IIFE string. The runtime's sandbox-store imports this string and passes
// it via the web-platform plugin's `bundleSource` config so the sandbox
// worker evaluates the polyfills inside QuickJS.
//
// Register in `packages/runtime/vite.config.ts` (production bundle) and
// in the top-level `vitest.config.ts` (test runs). The sandbox worker
// itself no longer consumes `virtual:sandbox-polyfills` directly — only
// downstream runtime builds need the virtual module resolved.
