...
import { renderEntryList, renderHeaderStats, renderTypeFilter, renderEventTypeCheckboxes } from "./views/list.js";
import { renderTimeline } from "./views/timeline.js";

const require = createRequire(import.meta.url);
const alpineJs = readFileSync(require.resolve("alpinejs/dist/cdn.min.js"), "utf-8");
const htmxJs = readFileSync(require.resolve("htmx.org/dist/htmx.min.js"), "utf-8");

const IMMUTABLE_CACHE = "public, max-age=31536000, immutable";
const VALID_STATES = new Set(["pending", "failed", "done"]);

async function handleListFragment(eventStore: EventStore, fragment: string, type: string | undefined, eventTypesParam: string | undefined): Promise<string | null> {
	if (fragment === "stats") {
		return renderHeaderStats(await getHeaderStats(eventStore));
	}
	if (fragment === "triggerTypes") {
		return renderTypeFilter(await getDistinctEventTypes(eventStore), type ?? "");
	}
	if (fragment === "eventTypes") {
		return renderEventTypeCheckboxes(await getAllEventTypes(eventStore), eventTypesParam ? eventTypesParam.split(",") : []);
	}
	return null;
}
...
function dashboardMiddleware(eventStore: EventStore): Middleware {
	...
	async (c) => {
		...
		if (fragment) {
			const html = await handleListFragment(eventStore, fragment, type, eventTypesParam);
			if (html !== null) {
				return c.html(html);
			}
		}
		...
		const result = await listCorrelations(eventStore, {
			state: state && VALID_STATES.has(state) ? (state as AggregateState) : undefined,
			type: type || undefined,
