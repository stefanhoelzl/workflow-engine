...
| `prod/`       | K8s read (ephemeral block re-fetch)      | `TF_VAR_dynu_api_key`, `TF_VAR_github_oauth_client_id`, `TF_VAR_github_oauth_client_secret`    |
...
- `cluster/`: `acme_email`
- `prod/`: `domain`, `auth_allow`, `image_tag`
...
- **Prod** (operator-driven): bump `image_tag` in `envs/prod/terraform.tfvars`, run `tofu -chdir=infrastructure/envs/prod apply`. The existing `Release` GHA workflow (on `release` tag push) builds + pushes the image and prints the tag to use.
