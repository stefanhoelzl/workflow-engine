...
- **Runtime tests** (`packages/runtime/`): executor serialization per workflow; cross-workflow parallelism; persistence of `trigger.request → trigger.response/trigger.error`; EventStore bootstrap from archive; recovery sweeps abandoned `pending/` entries by emitting a synthetic terminal `trigger.error` event whose `error.kind` is `"engine_crashed"`, which Persistence then archives and cleans up.
...
5. **Lifecycle events**: the sandbox emits a `trigger.request` event at start and `trigger.response` / `trigger.error` at end; the executor's `sb.onEvent` receiver widens each with runtime metadata (`tenant`, `workflow`, `workflowSha`, `invocationId`, plus `meta.dispatch` on `trigger.request` only) and forwards via `await bus.emit(event)`. The bus dispatches synchronously through the ordered consumer list: persistence (writes `pending/<id>.json` on `trigger.request`, writes `archive/<id>.json` + removes pending on `trigger.response`/`trigger.error`), EventStore (DuckDB in-memory index update on terminal events), logging-consumer (one structured pino line per lifecycle event: `id`, `workflow`, `trigger`, `ts` from `event.at`, plus `error` on failure; message names are `invocation.started` / `invocation.completed` / `invocation.failed`).
...
Startup wires components in this order before binding the HTTP port: storage backend → bus + consumers (EventStore bootstraps its index from `archive/` at init) → `SandboxFactory` → `SandboxStore` → executor → trigger backends (all `start()`ed) → `WorkflowRegistry` + `registry.recover()` (reconfigures every started backend with persisted tenants) → `recover({backend}, bus)` pending sweep (scans `pending/`, emits a synthetic `trigger.error` lifecycle event with `error.kind = "engine_crashed"` for each entry, lets the persistence + EventStore + logging consumers reconcile the index) → HTTP server bind.
...
- **sdk/plugin**: Single-pass Vite build that emits one tenant tarball `dist/bundle.tar.gz` containing root `manifest.json` (`{ workflows: [...] }`) plus one `<name>.js` per workflow at the tarball root. Discovers `defineWorkflow` / `action` / `httpTrigger` / `cronTrigger` / `manualTrigger` exports by brand symbol equality on the export value. Resolves workflow name from `defineWorkflow({name})` or from the source file's filestem. Derives input/output JSON Schema from the Zod schemas via `z.toJSONSchema()`. Enforces TypeScript type checking on production builds. Fails the build on: zero-or-many `defineWorkflow` exports per file, missing schemas, missing handlers, duplicate or unnamed action exports.
...
- **runtime**: HTTP server (Hono), workflow registry (metadata-only; owns per-tenant workflow manifests + bundle sources; `registerTenant(tenant, files, {tarballBytes})` persists `workflows/<tenant>.tar.gz` and atomically replaces the tenant's metadata; backend-plugin host for `TriggerSource` implementations), sandbox store (keyed by `(tenant, workflow.sha)`; lazily constructs sandboxes on first use via the sandbox factory, composes the production plugin catalog, importing each plugin module via the `?sandbox-plugin` vite query and spreading the `host-call-action` descriptor together with Ajv validator sources compiled on the main thread via `compileActionValidators(manifest)`, holds sandboxes for process lifetime — no eviction, re-upload orphans the old sandbox for any in-flight invocation; stamps `tenant`/`workflow`/`workflowSha`/`invocationId` on every event via `onEvent`), executor (per-`(tenant, workflow.sha)` runQueue, resolves sandbox via store, dispatches via `sandbox.run`, stamps `meta.dispatch` on `trigger.request` events only, lifecycle event emission via bus), EventBus + consumers (persistence, event-store with tenant column + nullable `meta JSON`, logging), HTTP trigger middleware (parses mechanical `/webhooks/<tenant>/<workflow>/<export-name>` URL, delegates to fire closure), trigger UI (`/trigger/*`, authenticated, server-synthesizes `HttpTriggerPayload` for HTTP descriptors), API (`/api/*` including `POST /api/workflows/<tenant>` upload behind `requireTenantMember()`), recovery (`recover()`), startup wiring (`main.ts` constructs the executor, starts every `TriggerSource` backend, then boots the registry from storage which reconfigures the started backends, then runs the `pending/` recovery sweep, then binds the HTTP server). No scheduler, no event-source, no work-queue, no context module, no `WorkflowRunner` abstraction.
