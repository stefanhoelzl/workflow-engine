...
- **DuckDB-backed durable archive**: invocation events live in a plain DuckDB database file at `<PERSISTENCE_PATH>/events.duckdb` with `PRIMARY KEY (id, seq)` and a secondary index on `(owner, repo)`. EventStore buffers in-flight events in an in-memory accumulator and commits the full event list as one auto-committed multi-row INSERT on terminal kinds (`trigger.response`, `trigger.error`, plus single-leaf kinds `trigger.exception` / `trigger.rejection` / `system.upload`). SIGKILL deliberately loses uncommitted in-flight invocations; SIGTERM drains gracefully via synthetic `trigger.error{kind:"shutdown"}` terminals. Single-writer is a deployment contract enforced structurally by running exactly one Quadlet unit per env (no orchestrator can spawn a second); DuckDB's exclusive file lock provides defence-in-depth.
- **Interface-first**: Storage is abstracted behind `StorageBackend` (FS-only) with `init`, `read`, `write`, `list` methods. Used by `WorkflowRegistry` for upload bundles; EventStore takes the persistence root path directly. New consumers of invocation events do not exist by design — the executor calls EventStore directly.
...
- **Runtime tests** (`packages/runtime/`): executor serialization per workflow; cross-workflow parallelism; EventStore commit-per-terminal against DuckDB; SIGTERM drain synthesises `trigger.error{kind:"shutdown"}` for any in-flight invocation; SIGKILL deliberately loses uncommitted in-flight invocations.
- **Integration tests**: workflow build → bundle load → registry register → webhook POST → executor.invoke → terminal commits to DuckDB → cold-start over WAL.
...
**Example workflows (two, distinct jobs).** `packages/sdk/example.ts` is the canonical full-surface authoring reference: it exercises every author-facing SDK surface (including infra-only `imapTrigger`/`wsTrigger`), ships in the SDK npm tarball, and is bundle-validated in CI (`packages/sdk/src/example.test.ts` runs `wfe build`, never uploads — bundling typechecks without executing, so infra-only triggers are covered). `workflows/src/demo.ts` is the runnable `pnpm dev` fixture: a subset that actually runs locally (no `imapTrigger` — dev starts no IMAP server), auto-uploaded on every boot for the dashboard. SDK-surface changes update `example.ts`; see `CLAUDE.md` §Example workflows.
...
Startup wires components in this order before binding the HTTP port: storage backend → EventStore (opens `<PERSISTENCE_PATH>/events.duckdb`; constant-time regardless of historical event count) → `SandboxFactory` → `SandboxStore` → executor (constructed with `eventStore` + `logger`) → trigger backends (all `start()`ed) → `WorkflowRegistry` + `registry.recover()` (reconfigures every started backend with persisted tenants) → HTTP server bind. There is no recovery scan — uncommitted in-flight invocations on unclean termination (SIGKILL, OOM, force-delete) are deliberately lost; SIGTERM drains gracefully via synthetic `trigger.error{kind:"shutdown"}` terminals committed before exit.
...
├── sdk/              # @workflow-engine/sdk (authoring API + vite plugin)
