...
- **Pluggable trigger sources**: one `TriggerSource` implementation per kind (currently only `http`). Sources plug into the `WorkflowRegistry` as a plugin host; on every workflow state change the registry synchronously pushes a kind-filtered view to each source via `reconfigure(view)`. Sources call the shared kind-agnostic `Executor` (returns `{ ok, output } | { ok: false, error }`).
...
- **runtime**: HTTP server (Hono), workflow registry (metadata-only; owns per-tenant workflow manifests, bundle sources, and HTTP trigger index; `registerTenant(tenant, files, {tarballBytes})` persists `workflows/<tenant>.tar.gz` via temp+move and atomically replaces the tenant's metadata; `lookup(tenant, workflow, path, method) → {workflow, triggerName, validator, bundleSource}`), sandbox store (keyed by `(tenant, workflow.sha)`; lazily constructs sandboxes on first use via the sandbox factory, wires `__hostCallAction` closure + Ajv input validators per workflow, holds sandboxes for process lifetime — no eviction, re-upload orphans the old sandbox for any in-flight invocation), executor (per-`(tenant, workflow.sha)` runQueue, resolves sandbox via store, dispatches via `sandbox.run`, lifecycle event emission via bus), EventBus + consumers (persistence, event-store with tenant column, logging), HTTP trigger middleware (parse `/webhooks/<tenant>/<name>/<path>` + registry lookup + validate + delegate + shape response), trigger UI (`/trigger/*`), API (`/api/*` including `POST /api/workflows/<tenant>` upload with `isMember` membership gate), recovery (`recover()`), startup wiring (`main.ts` boots registry from storage; `WORKFLOW_DIR` bootstrap removed). No scheduler, no event-source, no work-queue, no context module, no `WorkflowRunner` abstraction.
