...
**Reason:** DuckLake-specific. Plain DuckDB manages WAL/checkpoint internally — auto-checkpoint when WAL exceeds `wal_autocheckpoint` (~16 MiB) and fold-on-graceful-close. No timer, no thresholds, no operator-tunable knobs.

**Migration:** `EVENT_STORE_CHECKPOINT_INTERVAL_MS`, `EVENT_STORE_CHECKPOINT_MAX_INLINED_ROWS`, and `EVENT_STORE_CHECKPOINT_MAX_CATALOG_BYTES` env vars are removed. Operators who set them in Helm/compose values SHALL remove the references; the runtime ignores unknown env vars.
