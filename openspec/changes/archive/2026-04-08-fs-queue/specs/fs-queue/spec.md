...
1. Atomic-write the terminal state file to `pending/`
2. Call `super.ack(eventId)` or `super.fail(eventId)` to update in-memory state
3. Move all `*_evt_<id>.json` files from `pending/` to `archive/`, lowest counter first

All steps SHALL be awaited before returning.

#### Scenario: Ack persists, updates memory, then archives

- **GIVEN** an event `"evt_abc"` is in processing state and `000001_evt_abc.json` exists in `pending/`
- **WHEN** `ack("evt_abc")` is called
- **THEN** a new file `<counter>_evt_abc.json` (state: done) SHALL be written to `pending/`
- **AND** the in-memory state SHALL be updated to done
...
#### Scenario: Fail persists, updates memory, then archives

- **GIVEN** an event `"evt_abc"` is in processing state and `000001_evt_abc.json` exists in `pending/`
- **WHEN** `fail("evt_abc")` is called
- **THEN** a new file `<counter>_evt_abc.json` (state: failed) SHALL be written to `pending/`
- **AND** the in-memory state SHALL be updated to failed
