...
<!-- ═══════════════════════════════════════════════════════ -->
<!-- Pulumi stack (all requirements removed)                -->
<!-- ═══════════════════════════════════════════════════════ -->

### Requirement: Pulumi project in infrastructure/
**Reason**: Replaced by OpenTofu modules in `infrastructure/dev/` and `infrastructure/modules/`
**Migration**: Run `pulumi destroy` and `pulumi stack rm dev`, then delete Pulumi files

### Requirement: ESM TypeScript configuration
**Reason**: Pulumi TypeScript project no longer exists
**Migration**: No action needed — infrastructure is now HCL

### Requirement: Dev stack targets local Docker
**Reason**: Replaced by kind Kubernetes cluster managed by OpenTofu
**Migration**: Use `tofu apply` in `infrastructure/dev/`

### Requirement: Stack config is the single source of truth
**Reason**: Replaced by `terraform.tfvars` and `dev.secrets.auto.tfvars`
**Migration**: Extract secrets via `pulumi config --show-secrets` before deletion

### Requirement: Secrets stored as Pulumi encrypted secrets
**Reason**: Replaced by gitignored `*.secrets.auto.tfvars` files with `sensitive = true` variables
**Migration**: Extract secrets to `dev.secrets.auto.tfvars` before deleting Pulumi

### Requirement: Pulumi Cloud state backend
**Reason**: Replaced by local state backend (`backend "local" {}`)
**Migration**: No action needed — Pulumi Cloud state can remain (orphaned)

### Requirement: Image build uses docker-build provider
**Reason**: Replaced by `image/local` module using `podman build` via `terraform_data`
**Migration**: Use `tofu apply` which builds the image automatically

### Requirement: Containers managed by docker provider
**Reason**: Replaced by Kubernetes Deployments managed by `hashicorp/kubernetes` provider
**Migration**: Use `tofu apply` to create K8s deployments

### Requirement: App container configuration
**Reason**: Replaced by `workflow-engine/modules/app` OpenTofu module with S3 storage
**Migration**: App now uses `PERSISTENCE_S3_*` env vars instead of `PERSISTENCE_PATH`

### Requirement: Proxy container configuration
**Reason**: Caddy replaced by Traefik deployed via Helm in `workflow-engine/modules/routing`
**Migration**: Routing rules preserved as Traefik IngressRoute CRDs

### Requirement: OAuth2-proxy container configuration
**Reason**: Replaced by `workflow-engine/modules/oauth2-proxy` OpenTofu module
**Migration**: Same oauth2-proxy image and config, now as K8s Deployment

### Requirement: Deploy and destroy npm scripts
**Reason**: Replaced by `tofu apply` and `tofu destroy` commands
**Migration**: Remove `infra:update` and `infra:destroy` scripts from root `package.json`

<!-- ═══════════════════════════════════════════════════════ -->
<!-- Caddy reverse proxy (all requirements removed)         -->
<!-- ═══════════════════════════════════════════════════════ -->

### Requirement: Caddyfile exists at infrastructure/Caddyfile
**Reason**: Replaced by Traefik IngressRoute CRDs in routing module
**Migration**: Delete `infrastructure/Caddyfile`

### Requirement: Local HTTPS on localhost
**Reason**: TLS now handled by Traefik's default self-signed certificate in kind cluster
**Migration**: Traefik serves self-signed TLS automatically

### Requirement: Dashboard routes proxied to app
**Reason**: Replaced by Traefik IngressRoute with `PathPrefix('/dashboard')` and ForwardAuth middleware
**Migration**: Same routing behavior, expressed as Traefik CRDs instead of Caddyfile directives

### Requirement: Webhook routes proxied to app
**Reason**: Replaced by Traefik IngressRoute with `PathPrefix('/webhooks')`
**Migration**: Same routing behavior

### Requirement: OAuth2 routes proxied to oauth2-proxy
**Reason**: Replaced by Traefik IngressRoute with `PathPrefix('/oauth2')`
**Migration**: Same routing behavior

### Requirement: Caddy auto-reloads on Caddyfile changes
**Reason**: No longer applicable — Traefik configuration is declarative via K8s CRDs, not file-based
**Migration**: Changes to routing are applied via `tofu apply`

### Requirement: Unmatched paths return 404
**Reason**: Traefik returns 404 by default for unmatched routes
**Migration**: No action needed — Traefik default behavior
