...
The instance SHALL additionally attach one persistent Block Storage volume for the prod app env via `additional_volume_ids`, declared as a separate `scaleway_block_volume` resource (see *Per-env persistence on dedicated block volumes*). Because it is a standalone resource, attaching it is an in-place `additional_volume_ids` update (a server stop/start), NOT a VPS replacement, and the root volume's `size_in_gb`/`volume_type` SHALL be left unchanged by this attachment. Editing the root volume's local-SSD `size_in_gb` (which forces instance recreation) is NOT part of attaching the data volume.
...
- **AND** the instance SHALL have exactly one Block Storage volume attached via `additional_volume_ids` (the prod data volume)

#### Scenario: Attaching the data volume does not replace the VPS

- **GIVEN** an existing provisioned VPS with the prod data volume not yet attached
- **WHEN** the `scaleway_block_volume` resource is added to `additional_volume_ids` and `tofu apply` runs
...
Per-tenant container-runtime user accounts SHALL be declared in the project's managed-users configuration and created, updated, and removed via the in-place convergence mechanism — NOT via cloud-init. Specifically: `wfe-prod` and `wfe-caddy` (one per Quadlet tenant) SHALL each be managed entries with:
...
- **GIVEN** all entries for a tenant `wfe-experimental` (user, dirs, env file, Quadlet) are removed from source in the same apply
- **WHEN** `tofu apply` runs
- **THEN** the Quadlet's service SHALL stop first (dependents destroyed before user)
- **AND** the `wfe-experimental` user SHALL be removed via `userdel --remove`
...
- `caddy.container` (under `/home/wfe-caddy/.config/containers/systemd/`) referencing the Caddy image, with `Network=host` (Caddy must reach `127.0.0.1:8081` to proxy to the prod app; under bridge networking, the container's own loopback would not see those upstreams). `PublishPort=` is therefore omitted (host networking binds 80/443/443/udp directly via the user namespace, permitted by the `net.ipv4.ip_unprivileged_port_start=80` sysctl). Volumes: `/srv/caddy/data:/data:Z`, `/srv/caddy/config:/config:Z`, `/etc/caddy/Caddyfile:/etc/caddy/Caddyfile:ro,Z`.
...
Staging is NOT a VPS tenant — it runs on Bunny Magic Containers (see the `bunny-staging` capability), so no `wfe-staging.container` Quadlet is rendered.
...
#### Scenario: Both units start after apply

- **GIVEN** `tofu apply` has completed against a fresh VPS
- **WHEN** the operator queries each tenant's user-mode systemd (e.g. `sudo runuser -u wfe-prod -- env XDG_RUNTIME_DIR=/run/user/$(id -u wfe-prod) /bin/systemctl --user is-active wfe-prod.service`, similarly for wfe-caddy)
- **THEN** both SHALL print `active`
...
- **AND** the new file SHALL be installed at `/home/wfe-prod/.config/containers/systemd/wfe-prod.container`
...
The hostname SHALL be composed from a `base_domain` variable (default `stho.net`) as `workflow-engine.${base_domain}`, so a future domain change is a single-variable edit. Staging's public hostname is served by Bunny Magic Containers, not the VPS, so no staging site block is rendered (see the `bunny-staging` capability).
...
- **WHEN** the rendered `wfe-prod.container` is inspected
