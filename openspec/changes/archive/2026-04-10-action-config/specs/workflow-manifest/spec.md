...
The SDK SHALL export a `ManifestSchema` Zod object for validating `manifest.json` files. The `actions[].env` field in the schema SHALL be `z.record(z.string())`. The runtime SHALL parse every manifest through `ManifestSchema` at load time.

#### Scenario: Valid manifest passes validation
- **WHEN** a well-formed `manifest.json` with `env` as `Record<string, string>` is parsed through `ManifestSchema`
