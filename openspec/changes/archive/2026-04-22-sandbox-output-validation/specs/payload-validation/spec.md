...
- **AND** the trigger's descriptor.outputSchema is the JSON Schema form of `z.unknown()` (matches any value)
- **WHEN** the `fire` closure resolves
...
- FROM: `### Requirement: Action output validated in-sandbox by the SDK wrapper`
- TO: `### Requirement: Action output validated at the host-side bridge handler`

## MODIFIED Requirements

### Requirement: Action output validated at the host-side bridge handler

The sdk-support plugin's guest-function-descriptor handler for `__sdkDispatchAction` SHALL, after the captured `handler(input)` guest callable resolves with a raw value, validate the raw value against the action's declared output JSON Schema using a host-side Ajv-compiled validator exported by the `host-call-action` plugin (`validateActionOutput(name, raw)`). Validation SHALL run on the host before the dispatcher handler resolves to the guest caller. Validation failure SHALL throw a ValidationError (carrying the same `issues` array shape used by input validation) back into the guest caller; the guest SHALL observe the throw as a rejection of its `await __sdk.dispatchAction(...)` call.
