...
Every authenticated UI surface SHALL render an in-page tab strip between the sidebar and the main content area. The strip SHALL contain exactly two tabs — `Invocations` and `Trigger` — corresponding to the `/invocations/*` and `/trigger/*` URL prefixes. The tab matching the current URL prefix SHALL render in an "active" visual state; the other tab SHALL render in an inactive state.

Each tab SHALL be a real anchor whose `href` swaps the URL prefix while preserving the rest of the path. Given the current URL `/<surface>/<rest>` (where `<rest>` is everything after the surface segment, possibly empty), the Invocations tab's `href` SHALL be `/invocations/<rest>` and the Trigger tab's `href` SHALL be `/trigger/<rest>`. A tab click is therefore a pure surface swap that keeps the user's selected scope (owner / repo / workflow / trigger) intact.
...
- **WHEN** any authenticated UI surface is rendered (`/invocations`, `/invocations/<owner>`, `/invocations/<owner>/<repo>`, `/invocations/<owner>/<repo>/<workflow>`, `/invocations/<owner>/<repo>/<workflow>/<trigger>`, and the symmetric `/trigger/*` paths)
- **THEN** the response body SHALL contain a tab strip with exactly two tabs labelled `Invocations` and `Trigger`
...
- **AND** the `Invocations` tab SHALL render in the inactive state
...
- **AND** the `Invocations` tab's `href` SHALL be `/invocations/acme/foo/deploy/run`
