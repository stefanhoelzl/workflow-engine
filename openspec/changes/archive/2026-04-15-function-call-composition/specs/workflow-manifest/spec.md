...
**Reason**: The v0 manifest shape is incompatible with the function-call composition model. Events are removed; per-action `on`/`emits`/`module`/`env` fields are removed; trigger `response` config is removed; per-workflow bundle replaces per-action bundles.

**Migration**: Rebuild workflows against the new vite-plugin. The new manifest shape is captured in the v1 manifest requirement above.

### Requirement: ManifestSchema validation

**Reason**: Schema reflects the v0 manifest shape, including required `events` array and per-action `on`/`emits`/`module`/`env` fields that no longer exist.

**Migration**: Use the v1 `ManifestSchema` from the SDK.
