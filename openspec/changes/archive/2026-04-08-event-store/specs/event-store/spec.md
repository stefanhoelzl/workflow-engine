...
`bootstrap(events, options)` SHALL INSERT all provided events into the events table. The `latest` option SHALL be ignored — EventStore inserts all events regardless of the `latest` flag.

#### Scenario: Bootstrap inserts all events

- **GIVEN** an EventStore
- **WHEN** `bootstrap([evt1_pending, evt1_processing, evt1_done], { latest: false })` is called
- **THEN** three rows exist in the events table

#### Scenario: Bootstrap with latest flag still inserts all

- **GIVEN** an EventStore
- **WHEN** `bootstrap([evt1_done, evt2_pending], { latest: true })` is called
- **THEN** two rows exist in the events table

#### Scenario: Bootstrap with empty array

- **GIVEN** an EventStore
- **WHEN** `bootstrap([], { latest: true })` is called
