...
### Requirement: Form submission via htmx.ajax()
The system SHALL provide a global `submitEvent` function that reads the Jedison form value and submits it via `htmx.ajax()`.

#### Scenario: Submit button triggers ajax call
- **WHEN** the user clicks the submit button for an event
- **THEN** `jedison.getValue()` is called on the cached instance
- **THEN** `htmx.ajax('POST', '/trigger/:eventType', ...)` is called with the JSON body
