...
The scheduler SHALL run a loop that dequeues events, matches them to actions, executes the handler, and acknowledges or fails the event.
...
- **THEN** the event is marked as processing
- **AND** the matching action's handler is called with the event
