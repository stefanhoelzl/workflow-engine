# Workflow Secrets Specification

## Purpose

Expose the user-facing workflow-secret feature on top of the `workflow-env-runtime` and `secrets-crypto-foundation` capabilities. Workflow authors declare secrets via `env({ secret: true })` (sealed at CLI upload, decrypted in the runtime, surfaced inside the guest as plain strings on `globalThis.workflow.env`) and register author-computed plaintexts via `secret(value)` at runtime. The runtime's `secrets` plugin owns the literal-plaintext scrubber that redacts known plaintexts from outbound `WorkerToMain` messages (events, `done` payloads, logs) before they reach the host bus. Fetch egress to third-party APIs is intentionally NOT scrubbed — plaintext MUST reach the destination server so credentials work.
...
Decryption of `manifest.secrets` into the `plaintextStore` SHALL happen once per sandbox construction in `sandbox-store` via `decryptWorkflowSecrets`; the executor no longer performs crypto. The decrypted `plaintextStore` SHALL be delivered into every run's `ctx` alongside `envStrings`.

The `onPost` hook implements the literal-plaintext scrubber per SECURITY.md R-11. Fetch's on-wire network egress (the HTTP request body/headers crossing `hardenedFetch` to third-party APIs) SHALL NOT be scrubbed — plaintext credentials are expected to reach their destination; only `WorkerToMain` messages returning back into the host bus SHALL be redacted.
