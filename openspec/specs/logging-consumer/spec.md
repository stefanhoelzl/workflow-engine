...
The logging consumer SHALL handle only the three invocation lifecycle kinds (`trigger.request`, `trigger.response`, `trigger.error`). All other bus event kinds — `action.*`, `fetch.*`, `timer.*`, `console.*`, `wasi.*`, `system.*` — SHALL be ignored (no log entry emitted). Those kinds are captured by the EventStore for the dashboard and would be too verbose for the structured stdout log stream.
...
#### Scenario: system.* event is not logged

- **WHEN** the consumer receives an event with `kind: "system.*"`
- **THEN** the consumer SHALL NOT emit a log entry for that event
