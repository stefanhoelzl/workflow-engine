...
The fragment SHALL contain at most 50 items, rendered in FIFO order (oldest first — i.e. the next item to be dequeued appears first). Each item SHALL be rendered as a block containing the item's JSON value mounted in the shared interactive JSON-tree component (see `ui-foundation`).
...
- **AND** the response SHALL contain 50 item blocks rendered in FIFO order
...
- **AND** the response SHALL contain 50 further item blocks
...
- **THEN** the response SHALL contain 20 item blocks
...
- **THEN** the response SHALL contain zero item blocks
- **AND** the response SHALL NOT contain a "Load more" control
...
Each item within an items fragment SHALL be rendered using the shared interactive JSON-tree component defined in `ui-foundation`. The component's default state for queue items SHALL be fully expanded. The fragment SHALL NOT render items as plain `<pre>` blocks containing `JSON.stringify` output.

#### Scenario: Items render via the JSON-tree component

- **GIVEN** queue `jobs` contains item `{"orderId": 42, "items": [{"sku": "X"}]}`
- **WHEN** the items fragment for the queue is requested
- **THEN** the response SHALL contain the item rendered as the shared JSON-tree component's markup (e.g. an element bound to the registered Alpine component name)
