...
- **THEN** responses to `/invocations`, `/livez`, `/api/*`, `/webhooks/*`, and `/static/*` do not carry a `Strict-Transport-Security` header
...
HTML rendered by the runtime (invocations view, trigger UI, error pages, sign-in page template) MUST NOT contain any of the following:
...
- **WHEN** the runtime renders any HTML response (`/invocations`, `/invocations/<owner>/<repo>`, `/invocations/<owner>/<repo>/<id>/flamegraph`, `/trigger`, error page bodies)
