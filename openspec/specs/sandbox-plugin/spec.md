...
The worker→main host-call channel SHALL be treated as a trust boundary, given as explicit treatment in `SECURITY.md §2`. (This requirement is **moved** into `sandbox-plugin` from the retired `host-security-baseline` capability — the Scaleway VPS host posture is removed in this change — because it is an app/sandbox security boundary, unaffected by where the app is hosted.)
