...
Define the agent-facing documentation surface the platform ships so that an LLM authoring or deploying a workflow can discover and consume version-matched docs. This surface comprises the shipped `example.ts` full-surface authoring reference, an agent-facing SDK `README.md`, TSDoc doc-comments on every author-facing SDK export (surfaced on LSP hover and in the emitted `.d.ts`), and the static runtime `/llms.txt` index that points agents at the version-matched docs (unpkg `@latest`, or the local `node_modules` copy when the SDK is already installed).
