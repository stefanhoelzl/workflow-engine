# Secrets Public-Key API Specification

## Purpose

Expose the runtime's current primary X25519 public key and its fingerprint to authenticated tenant members so the `wfe upload` CLI can seal workflow secrets against it before POSTing the bundle. The endpoint is the only surface that reveals key material; it exposes only the primary key, never retained decryption-only keys.
