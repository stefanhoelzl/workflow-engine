...
Define the `StorageBackend` interface (filesystem and Bunny Edge Storage implementations) that the runtime uses for tenant bundle persistence under `workflows/<owner>/<repo>.tar.gz`. It is solely a bytes store for tenant bundles — EventStore and QueueStore own the libSQL database directly (via `DATABASE_URL`), so there are no `pending/` or `archive/` entries here. Owns atomicity, the `NotFoundError` read-miss contract, opaque-key handling, and the backend-selection logic: `createStorage` dispatches on the `STORAGE_BACKEND` env var and owns per-backend required-config validation.
## Requirements
