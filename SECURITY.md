...
- **Per-Quadlet memory ceilings** — each app unit caps at `350m` via `PodmanArgs=--memory=` on the container *payload* cgroup (visible inside the container as `/sys/fs/cgroup/memory.max`, so memory-auto-sizing software like DuckDB sizes against the real budget instead of host RAM); Caddy caps at `80M` via `[Service] MemoryMax=` (no auto-sizing software inside). A runaway workload is OOM-killed within its own unit's cgroup, not its neighbour's. Mitigates **I5**.
